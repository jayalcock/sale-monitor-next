...
@patch("sale_monitor.services.price_extractor.PriceExtractor.extract_price", return_value=(7.77, 'manual'))
def test_check_all_endpoint(_mock_extract, tmp_path):
    client = make_client(tmp_path)
    # Add second enabled product
    add = client.post(
        "/api/product/add",
        json={
            "name": "Extra",
...
def test_product_history_includes_base_currency(_mock_extract, tmp_path):
    client = make_client(tmp_path)
    url = "https://example.com/w"
    # create one history point
    client.post("/api/product/check", json={"url": url})
    r = client.get("/api/product/history", query_string={"url": url, "days": 7})
    assert r.status_code == 200
    data = r.get_json()
    assert isinstance(data, list)
    assert len(data) >= 1
    # Ensure new fields exist
    assert "price_in_base" in data[0]
    assert "base_currency" in data[0]
